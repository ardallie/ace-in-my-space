...
**Tip:** start a new conversation and pass this report to `$ace:ace-agent-pre-planner` or a planning agent. If the Workload assessment splits the scope into multiple briefs, start one conversation per brief and run that brief's printed `Pre-planner invocation:` line. Do not pass this report directly to `$ace:ace-plan-start` -- it is not a pre-planner report, and its open questions would not surface there.
...
- Brief {N} -- {title} — covers {ids; mark foldable extras `(optional)`}; depends on {prior briefs or none}
  ...
  Pre-planner invocation: `$ace:ace-agent-pre-planner {report path} Scope: Brief {N} -- {title} ({ids}) only`
