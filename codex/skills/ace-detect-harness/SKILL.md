...
`../ace-detect-harness/models.md`; this file holds the detection procedure, the
...
line carries the literal marker `(ace-detect-harness, session cache)`; a skill that needs
...
On success, read `../ace-detect-harness/models.md` and print exactly one block:
...
tier: a space-separated model and effort, which a consumer splits into the spawn tool's `model` and `reasoning_effort`
fields (`models.md` records the accompanying spawn constraints).

On Codex:

```
Harness: codex  (ace-detect-harness, session cache)
...
- Tier prescriptions resolve at spawn time against the `Harness:` block in the session
  ...
  exposes `$ace:ace-detect-harness` as an invokable skill, invoking it performs exactly this
  and is equivalent.
- The block's `Tier-N` value substitutes for the reference. It is a space-separated pair: split it into the spawn tool's separate `model` and
