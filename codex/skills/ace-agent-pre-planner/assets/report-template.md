...
Stage: {n}/{m}
Source: {per `## Standard header fields` in `../agent-shared/report-conventions.md`}
...
[Why the shared `Source:` path-list rule is load-bearing here: `$ace:ace-plan-start`'s pre-planner
transform (`../ace-plan-start/references/pre-planner-transform.md`) copies the
...
**Tip:** start a new conversation and pass this report to `$ace:ace-plan-start` (or a planning agent).
