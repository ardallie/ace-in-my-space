---
name: ace-plan-start
description: Drafts a plan from a brief (slug, file, instruction text, pre-planner report, or GitHub issue), saves it to .ace/plans/, validates it via the ace-plan-validate subagent, applies findings via ace-plan-revisor into a v1 revision, then publishes the issue inline via the $ace:ace-report-publish pipeline (--publish no to skip).
---
**Invocation input:** `[--model <name or instruction>] [--skip-interview] [--publish yes|no] [<slug|file-path|report-path|issue> ...] [additional instructions]`
...
- $ace:ace-plan-start — uses the most recent actionable user message as the brief
- $ace:ace-plan-start <slug-or-path> [<slug-or-path> ...] — loads brief from file(s); concatenated peers
- $ace:ace-plan-start <slug-or-path> ... <additional instructions> — first non-resolving token + remainder become additional instructions
- $ace:ace-plan-start <path-to-ace-agent-pre-planner-report.md> — generates a plan from a pre-plan analysis report; resolved decisions and `### Confirmed` findings become constraints, `[blocking]` open-questions feed the Phase 3 interview; `[deferrable]` ones become drafter-directed 'resolve in the plan' constraint lines
- $ace:ace-plan-start <issue> — generates a plan from a GitHub issue (bare number or full URL). The issue body plus its comments are the sole source of truth for the run; a published pre-planner report is detected from the issue content and receives the same transform as the file form
- $ace:ace-plan-start [--model <name or instruction>] [--skip-interview] [--publish yes|no] <inputs ...> — flags first (in any order), positional second

## Context

Runs in either Codex collaboration mode and does not depend on Plan mode. The brief arrives as free text, local files, or a GitHub issue (Phase 1's issue route). A `ace-plan-drafter` subagent reads the codebase and writes the plan to a deterministic path under `.ace/plans/`.

The `ace-plan-drafter` supporting payload uses `exec_command`, `rg --files`, `rg`, and `apply_patch`, and is passed through `collaboration.spawn_agent.message`. The built-in Codex agent role does not enforce a per-path write restriction; the subagent's prompt is the binding constraint — it writes only the single plan file.

Beyond the pipeline's own artefacts, this skill depends on the following sibling files; a port of the plan pipeline must carry this set: `../../agents/ace-plan-drafter.md`, `../../agents/ace-plan-validate.md` (the subagent Phase 7 spawns directly — not the standalone `$ace:ace-plan-validate` skill at `../ace-plan-validate/SKILL.md`, which this skill never invokes or reads), `../../agents/ace-plan-revisor.md`, `../ace-report-publish/SKILL.md`, `../ace-run-interview/SKILL.md`, the shared files `../plan-shared/identifiers.md`, `../plan-shared/pre-planner-detection.md`, and `../plan-shared/resolution-markers.md`, this skill's own `references/issue-route.md` and `references/pre-planner-transform.md`, and the rules files `../../rules/environment.md`, `../../rules/principles.md`, and `../../rules/output-style.md`.
...
**Detection** (applies before the rules below): classify each resolved input file per the two-rule detection in `../plan-shared/pre-planner-detection.md` — read it now; it carries both arms and their precedence, and `$ace:ace-plan-validate` and `$ace:ace-plan-update` consume the same detection. A file the detection does not match falls through to the regular list rows. Issue inputs arrive pre-classified from Phase 1's issue route; the pre-planner staged filename matches the filename rule by construction, so no re-classification occurs.
...
Brief assembly — literal payload to the `ace-plan-drafter` subagent:
...
- **Issue input (plain brief)** — treated as a single-path input over the staged file, with one addition: the `Source: <staged path>` line is immediately followed by an `Issue: <issue URL>` line naming the origin. The drafter cites the issue, not the transport file (see `../../agents/ace-plan-drafter.md`)
...
- Pre-planner report input — derive the slug from the report's `Title:` field:
  ...
  - **Slug derivation** — apply the report-title slug rule in `../plan-shared/identifiers.md` (label strip, kebab-casing on every non-alphanumeric boundary, stop-word drop, 3-keyword truncation, with worked examples — single-sourced there).
  ...
  - **Sync note** — the title-derivation rule is single-sourced in `../plan-shared/identifiers.md`, which `../../agents/ace-plan-drafter.md` step 4 also consumes. The unit-marker append is parent-side only — for slug-derived inputs the drafter receives a fully-formed target path and never appends unit markers itself.
...
Compute the suffix per the 8-hex snippet in `../plan-shared/identifiers.md` (test output non-emptiness, not exit status — the rationale is recorded there).
...
Spawn-model prescriptions here are tier references (`Tier-1`-`Tier-3`); resolve them at spawn time per the consumer contract in `../ace-detect-harness/SKILL.md` — against the `Harness:` block already in context, invoking `$ace:ace-detect-harness` first if none is present; where the ace-detect-harness package is absent, resolve tiers against the harness's own model catalogue. This applies to every subagent spawn in this skill (Phases 5, 7, and 9).

Read `../../agents/ace-plan-drafter.md` in full, then spawn one subagent through `collaboration.spawn_agent`:
...
2. **`[Question]`-prefixed findings** — the classification criteria belong to `../../agents/ace-plan-validate.md` step 2, which the validator applies when writing the report; this phase only reads the prefix it left. For each, attempt autonomous resolution from the brief, the draft plan, and files already read in this run; a bounded codebase look-up (exec_command/rg) is permitted when a single targeted check settles the question unambiguously. If resolved, mark the finding `[Resolved]` internally and record the resolution.
3. **Remaining unresolved questions** — if `--skip-interview` was set in Phase 1, leave them unresolved (they stay as `[Question]` in the v1 footer audit trail and in the saved report). Otherwise, run a `request_user_input` interview following the rules in `../ace-run-interview/SKILL.md`, which govern the option format in full. Batch across multiple `request_user_input` calls if needed, presenting findings in report order with any `[Question]` finding that gates other findings first. The user may decline individual questions mid-interview; declined questions remain unresolved.

**Write resolutions back to the report.** For every finding resolved from context or by interview, execute the write-back per `../plan-shared/resolution-markers.md` — read it now; it carries the `apply_patch`-in-place shape and the single whole-file-write fallback. This makes chained-flow interview answers durable: `$ace:ace-plan-update --from-report` maps `[Resolved]` findings to `resolved-by-interview` and would otherwise lose them.
...
### Phase 9 — Pre-seed v1 and conditionally spawn ace-plan-revisor
...
**Pre-seed v1 (every run).** The three-step block below is the canonical pre-seed definition, so the footer shape has this single writer definition: `$ace:ace-plan-update` Phase 2A executes both this pre-seed block and the conditional-revisor-spawn block below by reference — reading them from this file — with its own inputs and its stated deltas (its state set excludes `resolved-from-context`; flow control stays with that skill).

1. `cp <draft path> <v1 path>` through `exec_command`. On Windows MINGW64, both paths must be in POSIX form (`/c/...`) per `../../rules/environment.md`.
2. Strip any existing `## Validation audit` block (the H2 header through end-of-file) from `<v1 path>` before appending. A `plan-v0` draft has no footer, so the strip is a no-op in this skill; it is load-bearing for the by-reference `$ace:ace-plan-update` execution, whose staged issue bodies already carry a published footer — without the strip each round trip accretes a duplicate `## Validation audit` H2, violating the byte-identical-structure invariant below and leaving the revisor's "do not edit the audit footer" contract ambiguous.
3. Append the audit footer to `<v1 path>`. The canonical structure: a blank line, the `## Validation audit` H2 header, a blank line, then `- ` bullets for `Validation report: <repo-relative report path>` and `Validated on: <ISO 8601 timestamp>`. Append a third bullet `Unresolved questions: <count>` only when the count is greater than zero. The footer must be byte-identical in structure across runs so downstream `$ace:ace-plan-implement` step 2 parsing (the `Validation report:` / `Validated on:` / `Unresolved questions:` bullets) remains agnostic to whether a revisor edit followed. (The `$ace:ace-report-publish` pipeline Phase 10 runs appends a final `- Issue: <url>` bullet to the local file after publication; that bullet is absent from the published issue body by construction.)

**Conditional revisor spawn.** The block runs from this lead-in through the **Post-spawn checks** paragraph below, which is its final part; the patch-failure paragraph after it is outside the block.
...
- Otherwise, read `../../agents/ace-plan-revisor.md` in full and spawn the revisor through `collaboration.spawn_agent`:
...
If a re-run produced `plan-v2` or higher (collision against an existing revision), the second and third lines reflect the actual version used (`val-v2`, `plan-v2`, etc.). Under `--publish no`, the final line is instead:
    ...
    Publish with:   $ace:ace-report-publish .ace/plans/{yyyyMMdd}-{HHmm}-plan-v1-{slug}-{8hex}.md
...
- The ace-plan-revisor subagent fails or the v1 file does not exist after it returns — see Phase 9.
