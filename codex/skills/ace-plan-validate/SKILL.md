...
---
**Invocation input:** `[--skip-interview] [<issue>|<file-path> ...] [additional instructions]`
...
- `$ace:ace-plan-validate` — uses issue number from conversation context
- `$ace:ace-plan-validate <issue>` — validates the plan from the specified issue number (e.g., `85`)
- `$ace:ace-plan-validate <file1> <file2> ...` — validates the plan from local files
...
- **No arguments** — if the plan is already in the conversation (from a prior `$ace:ace-plan-read`), stage it to a file: derive the slug and a fresh 8-hex per the report-title slug rule and 8-hex snippet in `../plan-shared/identifiers.md`, and use `apply_patch` to write the conversation-held plan text to `.ace/plans/{yyyyMMdd}-{HHmm}-plan-v0-{slug}-{8hex}.md`, then treat it as file-input from that path (the validator's contract is path-only — a file must exist). This staged file is not cleaned up either — same handling as the numeric-token branch. Otherwise look for an issue number in conversation context and stage it per `references/issue-staging.md`. If not found, report this and stop.

If a file path resolves but is not readable, report the error and stop. If the plan text is empty, report this and stop.

If a resolved plan file matches the pre-planner two-rule detection in `../plan-shared/pre-planner-detection.md` — read it now; it carries both arms and their precedence — stop with: `<path> is a pre-plan analysis, not a plan. Run $ace:ace-plan-start <path> to generate a plan from it.` The validator would otherwise treat the analysis as a plan (slug `pre-plan-analysis` from the H1).
...
- **Plan-file input not matching the convention** — generate a fresh 8-hex per the snippet in `../plan-shared/identifiers.md` (test output non-emptiness, not exit status); derive the slug from the plan's first H1 title (kebab-cased, ≤3 keywords, per the report-title slug rule in the same file); set `{n}` to `1`.
...
Read `../../agents/ace-plan-validate.md` in full, then spawn a validation subagent through `collaboration.spawn_agent` with a run-suffixed `task_name`, `agent_type: "default"`, `fork_turns: "none"`, `message` set to the complete payload followed by the run inputs, and the Tier-3 `model` plus `reasoning_effort` — a tier reference resolved at spawn time per the consumer contract in `../ace-detect-harness/SKILL.md`, against the `Harness:` block already in context, invoking `$ace:ace-detect-harness` first if none is present, and against the harness's own model catalogue where the ace-detect-harness package is absent; the parent session's default model is the fallback when tier resolution is unavailable. Pass the plan path (the file path for file-input; the temp-file path staged in Phase 1 for issue-input), the full report target path (computed in Phase 1), and any additional instructions collected in Phase 1 as inputs. The additional instructions bias which criteria and areas receive scrutiny; the chained `$ace:ace-plan-start` Phase 7 passes none. The subagent owns all codebase exploration and writes the report; the orchestrator does not pre-explore.
...
This phase collects user answers and writes them back into the report at `report_path` via in-place `apply_patch` edits.

**Run the interview tool:**

Use `request_user_input` when it is available in Plan mode; otherwise ask the same question in concise plain text to present questions interactively. Present findings in report order, with any `[Question]` finding that gates other findings first. The validator's `[Question]` format carries no blocking/deferrable tags — that vocabulary belongs to the analysis reports (pre-planner, arch-review, scope). For each question, follow the rules in `../ace-run-interview/SKILL.md`, which govern the option format. Draw each question's options from codebase evidence, trade-offs, or reasonable alternatives, so the user selects rather than composes a response from scratch — a rule specific to this phase, not carried by that file.
...
**Update the report** — execute the write-back per `../plan-shared/resolution-markers.md` — read it now; it carries the `apply_patch`-in-place shape, the audit-trail rule, the no-summary rule, and the single whole-file-write fallback. Inspect `report_path` once at the start of Phase 4 to confirm the file's structure; subsequent updates use `apply_patch` only. Skipped questions: no patch.
...
- If the composed temp path already exists with content differing from the fetched issue body, report the collision and the `$ace:ace-plan-validate <path>` file-input alternative, and stop (the clobber guard in `references/issue-staging.md`)
