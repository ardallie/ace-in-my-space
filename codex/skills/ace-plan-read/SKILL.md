...
---
**Invocation input:** `[<issue> ...] [then <follow-on>]`
...
- `$ace:ace-plan-read` - Uses the issue number from conversation context
- `$ace:ace-plan-read <issue>` - Loads context from the specified issue (e.g., `85`)
- `$ace:ace-plan-read <issue> <issue> ...` - Loads multiple issues; fetched one at a time and presented as a condensed cross-issue synthesis (see step 4)
- `$ace:ace-plan-read <issue> then <follow-on>` - Loads the issue, then handles the named follow-on (e.g. `$ace:ace-plan-read 957 then handle phase 1`); see step 5.

## Context

Use this to continue work started in a previous session. The issue contains the plan (description) and any handoff comments documenting progress. After completing work, use `$ace:ace-plan-handoff` to document the session.

## Task

1. Identify the issue number — or numbers, in the multi-issue form — from the arguments, or search the conversation for a GitHub issue URL or number, taking the most recent when the conversation carries more than one. If none is found, report this and stop. This rule is canonical; its aligned copies within the plan-workflow bundle are `$ace:ace-plan-handoff` step 1 and `$ace:ace-plan-update` Phase 1, and an amendment lands at all three. (`../ace-pr-create/SKILL.md` and `../ace-pr-response/SKILL.md` carry the same idiom undeclared, pending a relay-workflow-scoped pass. `$ace:ace-pr-read` step 2 phrases the same stop as "report usage and stop". The wording differs and the behaviour does not; both are in the set and neither is a drift to repair.)
