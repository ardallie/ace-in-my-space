...
- `../ace-agent-arch-review/SKILL.md` -- full; its SKILL.md appends the
...
- `../ace-agent-pre-planner/SKILL.md` -- full; its SKILL.md appends its local
...
Two aligned copies diverge deliberately and do not consume this file: `$ace:ace-agent-consultant`
compresses the context-package checklist to a question-scoped read and restates the
analyst instructions as its own researcher rules; `$ace:ace-agent-scope` replaces the checklist
with a knowledge-first posture on a smaller read budget, and carries its own panel
instructions. `$ace:ace-agent-code-review` is not a consumer either, and carries no aligned copy:
...
- Omit context-gathering directives that the analysis has fulfilled — instructions to fetch
  or read artefacts (e.g., "run $ace:ace-plan-read X", "inspect touched files") have been addressed
