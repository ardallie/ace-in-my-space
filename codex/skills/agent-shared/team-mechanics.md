...
subject is a SHA-pinned state, not the working tree). `$ace:ace-agent-consultant` does not consume
...
the consumer contract in `../ace-detect-harness/SKILL.md` — against the
`Harness:` block already in context, invoking `$ace:ace-detect-harness` first if none is present.
