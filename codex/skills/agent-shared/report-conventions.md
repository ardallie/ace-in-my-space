...
`$ace:ace-report-publish` composes the issue title from `Title:`, `Type:`, and `Stage:` via
...
- **`--publish no`** — do not create an issue. The final summary prints the saved report
  path plus the ready-to-run line `$ace:ace-report-publish {report path}`.
- **`--publish yes`** — publish the saved report per the `## Producer publish phase`
  section in `../ace-report-publish/SKILL.md`.
...
the ready-to-run `$ace:ace-report-publish {report path}` line — the saved report stands
regardless.
