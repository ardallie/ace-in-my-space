...
2. Write the report to `.ace/reports/{yyyyMMdd}-{HHmm}-agent-consultant-{suffix}.md`, where `{yyyyMMdd}` and `{HHmm}` are local machine time (`date +%Y%m%d` and `date +%H%M`) and `{suffix}` is the Phase 4 suffix (the exchange id). Create the `.ace/reports/` directory if it does not exist. If the write fails, retry once; if it still fails, output the report in full so the user can save it manually.
