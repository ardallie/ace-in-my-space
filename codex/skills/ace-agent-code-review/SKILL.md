---
name: ace-agent-code-review
description: Runs a multi-agent pre-PR code review (two or three specialists plus a sceptic) over the current branch, a PR, a remote or local branch, a commit, or a directory. Produces a severity-graded findings report (Critical/High/Moderate/Minor) consumed via $ace:ace-report-triage. Use for non-trivial changes before opening a PR -- not for trivial fixes, and not for architectural review, which is $ace:ace-agent-arch-review. Advisory only -- does not implement changes.
---
# Run a pre-PR agent code review

**Invocation input:** `[--model <name or instruction>] [--prior <report-path>] [--reviewers 2|3] [--checks full|cheap|skip] [--publish yes|no] [pr <number|URL>|origin/<branch>|branch <name>|commit <sha>|directory <path>]`

## Usage

- `$ace:ace-agent-code-review` — current branch vs default branch
- `$ace:ace-agent-code-review pr <number|URL>` — PR by number (current repo) or full URL
- `$ace:ace-agent-code-review origin/<branch>` — remote branch vs default branch
- `$ace:ace-agent-code-review branch <name>` — local branch vs default branch
- `$ace:ace-agent-code-review commit <sha>` — single commit diff (SHA must be at least 7 characters)
- `$ace:ace-agent-code-review directory <path>` — all files in the specified directory
- `$ace:ace-agent-code-review [--model <name or instruction>] [--prior <report-path>] [--reviewers 2|3] [--checks full|cheap|skip] [--publish yes|no] <scope ...>` — flags first (in any order), positional second
...
Pre-pass: strip leading `--model {value}`, `--prior {path}`, `--reviewers {value}`, `--checks {value}`, and `--publish {value}` flags per the flag pre-pass in `../agent-shared/input-conventions.md` — read that file now; it carries the `--model` grammar, the `--publish` grammar, and the residual-token warnings applied after parsing.
...
- `--checks` value, when supplied, selects the shared-verification tier defined in `../agent-shared/verification-checks.md`; accepted values `full`, `cheap`, and `skip`. If parsing fails (missing value or any other value), report the valid values and stop. If omitted, `cheap`. Drives Phase 3.
...
- **Other string** — use `request_user_input` when it is available in Plan mode, otherwise ask the same concise plain-text question, to clarify the intended scope. Present the input value and suggest the available keyword forms (`pr`, `branch`, `commit`, `directory`, `origin/`). The user's response may be one or more tokens; together they replace the original first token only and are re-parsed through the rules above; all subsequent original tokens remain additional instructions. If the response still matches no rule, report the unrecognised scope and stop — do not ask again.
...
**Budget warning** — when any count above exceeds the budget, apply `references/budget-overflow.md`; read it only at that point. It carries the truncation rule, the `request_user_input` options and their per-source availability, the source-specific narrowing advice, and the handling of each outcome.
...
**Shared verification** — before spawning, apply the selected `--checks` tier once, per the tier definitions in `../agent-shared/verification-checks.md` — read that file now. Local bindings:
...
**Package handoff** — per the shared handoff rule in `../agent-shared/team-mechanics.md`, passing the package path to each reviewer together with the pinned head SHA.
...
- Severity boundaries:
  ...
  - The four-tier scale is code-review-specific: findings here gate a merge decision, so they need finer impact resolution than the two-tier `[high]`/`[low]` severity vocabulary the analysis commands (`ace-agent-pre-planner`, `ace-agent-arch-review`) use to weight planning.
...
**Standard header fields.** Per `../agent-shared/report-conventions.md` `## Standard header fields`, with `Type: code-review` and no `Stage:` divergence.
...
Write the report to `.ace/reports/{yyyyMMdd}-{HHmm}-ace-agent-code-review-{suffix}.md` per the save procedure in `../agent-shared/report-conventions.md` `## Save report` (`{suffix}` is the Phase 3 hex suffix).

If the report contains `[Question]`-prefixed findings, proceed to the interview phase. Otherwise skip to Phase 8.

### Phase 7 — Interview

Follow the canonical interview procedure in `../agent-shared/interview.md` — read it now; it carries the run/collect/save mechanics and the unresolved-marker grammar. Two stated divergences: findings here carry no `[blocking]`/`[deferrable]` priority tags (that vocabulary belongs to the analysis reports — pre-planner, arch-review, scope), so present questions in report order (severity groups, most severe first), with any question that gates other findings first, batching gating and highest-severity questions in the first batch; and the unresolved marker is appended to `## Clarifications` (create it if needed) rather than to an `### Open questions` subsection.
...
- The command ends after Phase 8 (publication, or the printed `$ace:ace-report-publish` line under the default `--publish no`).
