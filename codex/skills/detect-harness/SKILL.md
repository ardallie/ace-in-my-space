---
name: detect-harness
description: "Prints the Codex agent model and reasoning-effort mapping for the suite's three capability tiers, read from models.md beside this skill. Invoke before spawning tiered agents when no valid cached resolution is present; see the cache check for reuse and invalidation rules."
---
...
settings live in `../detect-harness/models.md`.
...
establish that the mapped model or effort is stale.
...
- Use a spawn mode that honours model overrides, following the harness constraints
  ...
  assignment as inherited; do not claim the requested tier was applied.
- The block's `Tier-N` value substitutes for the reference. It is a space-separated
...
- Report surfaces record the resolved model and, on Codex, reasoning effort used for
  each assignment. When using default selection, report that overrides were omitted;
  do not invent a model or effort or use agent self-description as verification.
