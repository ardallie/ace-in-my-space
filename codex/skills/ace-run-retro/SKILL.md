...
Sequential invocations that form one piece of work (e.g. `$ace:ace-plan-read` followed by
`$ace:ace-plan-implement`) count as a single run. If nothing has run in this session and no argument
...
skill, or task name, e.g. `ace-plan-implement-883`. Never overwrite an existing report -- on a
path collision, append `-2` (then `-3`, etc.) to the run slug. The timestamp idiom and
collision-increment convention are canonical at `$ace:ace-plan-start` Phase 4
(`../ace-plan-start/SKILL.md`); this file's increment
...
Open the report with an H1 naming the run (e.g. `# Run retrospective -- $ace:ace-plan-implement 883`)
