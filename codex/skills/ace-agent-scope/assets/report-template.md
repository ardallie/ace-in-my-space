...
`../agent-shared/report-conventions.md`. No wildcards or prose descriptions.]
...
printed `$ace:ace-agent-pre-planner` invocation (the saved path and the published issue number are
equally valid). Do not pass this envelope to `$ace:ace-plan-start` -- it is not a plan brief; each
...
its update block and `../ace-agent-scope/references/sibling-routing.md`
...
knowledge-first with light grounding; each stage's `$ace:ace-agent-pre-planner` run re-derives
...
Pre-planner invocation: `$ace:ace-agent-pre-planner {report path} Scope: Stage {k} -- {title} ({goal ids}) only`
...
`../agent-shared/report-conventions.md` `## Standard header fields`. No
