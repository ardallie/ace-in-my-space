...
any interview, per `../agent-shared/sibling-exchange-protocol.md`.
...
`../ace-plan-route/SKILL.md`) or **owner-only** (a preference or priority call only
...
On option 2 — or if the gate's available `request_user_input` call and plain-text fallback both fail (record the unanswered
...
- **Compose the request envelope** by reproducing the skeleton in
  `../agent-shared/sibling-exchange-protocol.md` `## Request envelope` in
  ...
  one in `../agent-shared/report-conventions.md` `## Save report`, and this
...
- **Print the hand-off instructions** in the final summary: run
  `$ace:ace-agent-consultant {request file} {this report file}` in the sibling repository; carry
  its answers report back and pass it as an input to the next run here — a re-scope
  (`$ace:ace-agent-scope {this report path} {answers path}`) or the owning stage's pre-planner run
