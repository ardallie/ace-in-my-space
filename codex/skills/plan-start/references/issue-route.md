...
Executed from `$ace:plan-start` Phase 1 (`../plan-start/SKILL.md`) when the
...
2. **Classify.** Test the issue against four classification sets in order; the first matching set wins.
   ...
   - **Published plan** — any of:
     ...
     - Rationale: drafting a brand-new plan whose brief is an existing plan is always a mistyped command. This three-arm published-plan test is canonical here; `../pr-create/SKILL.md` step 5 carries an aligned copy for its `Closes #<n>` gate, and an amendment to either arm-set lands in both files.
