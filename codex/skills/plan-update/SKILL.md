...
- `$ace:plan-update <issue> --from-report <report-path>` — Applies a saved validation report to the issue via the `plan-revisor` subagent (no manual synthesis, no new interview). The flag may appear before or after the issue number — the Phase 1 pre-pass strips it wherever it sits.
...
1. **Reject pre-planner reports.** If the file matches the pre-planner two-rule detection in `../plan-shared/pre-planner-detection.md` — read it now; it carries both arms and their precedence — stop with: `<path> is a pre-plan analysis, not a validation report. Run $ace:plan-start <path> to generate a plan from it.` The per-bullet `## Findings` parser below would otherwise map the report's analysis findings to `applied` and fold them into the issue body as revisions.
...
2. Stage the current issue body to an intermediate file under `.ace/plans/`. Compute the intermediate-file path as follows:
   ...
   Write the body directly with `apply_patch` — the `plan-revisor` subagent reads files, not raw strings:
   ```bash
...
5. Once approved, update the issue from the staged file. Run the size pre-flight first by reference — execute the Phase 2A step 7 guard unchanged with `<staged path>` as the file — then update:
   ...
   This states a different rule from the pack-wide `--body-file` warrant canonical at `../pr-create/SKILL.md` step 6 (shell-quoting hazards in generated prose): here the content is already on disk, not a quoting-hazard avoidance — a later sync pass must not fold this note into that alignment set.
