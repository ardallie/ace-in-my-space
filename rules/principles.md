...
These rules apply to every interview surface -- `AskUserQuestion`, the `run-interview` skill, and ad-hoc prompts in commands, skills, and subagents -- not only to flows that route through `/ace:run-interview`.
