...
2. **Classify.** Test the issue against four classification sets in order; the first matching set wins.
   ...
   - **Published plan** — any of:
     ...
     - Disposition: stop with the redirect — `Issue #{n} is a published plan, not a brief. Use /ace:plan-read <n> to load it, /ace:plan-validate <n> to validate it, or /ace:plan-implement <n> to implement it.`
     - Rationale: drafting a brand-new plan whose brief is an existing plan is always a mistyped command. This three-arm published-plan test is canonical here; `${CLAUDE_PLUGIN_ROOT}/skills/pr-create/SKILL.md` step 5 carries an aligned copy for its `Closes #<n>` gate, and an amendment to either arm-set lands in both files.
   - **Review report** — any of:
     ...
     - Disposition: stop with the redirect — `Issue #{n} is a review report, not a brief. Run /ace:report-triage <n> to act on its findings, or /ace:agent-pre-planner <n> to derive a plan brief from it.`
   ...
   - **Scope envelope** — any of:
     ...
     - Disposition: stop with the redirect — `Issue #{n} is a scope envelope, not a plan brief. Run /ace:agent-pre-planner <n> (scoped per stage via the envelope's printed Scope: directives) to derive a plan brief from it.`
...
3. **Stage.** Write the body field verbatim to `.ace/plans/{yyyyMMdd}-{HHmm}-agent-pre-planner-issue{n}-{8hex}-staged.md` (pre-planner) or `.ace/plans/{yyyyMMdd}-{HHmm}-issue{n}-{8hex}-staged.md` (plain brief) — current local time, a fresh 8-hex per the snippet in `${CLAUDE_PLUGIN_ROOT}/skills/plan-shared/identifiers.md`, POSIX path form for the redirection per `${CLAUDE_PLUGIN_ROOT}/rules/environment.md`. If the write fails, report and stop — do not proceed against a partial staged file. Refuse to overwrite: if the path exists, append `-2`, `-3`, ... to the `-staged` suffix until a free name is found, mirroring `/ace:plan-update`. When the issue has comments, append them to the staged file after the body: a blank line, `---`, a blank line, an `## Issue comments` H2, then one `### {author} — {timestamp}` subsection per comment in chronological order, each body verbatim. The staged file is a transport artefact serving the drafter's by-reference `Source:` contract — not a second source of truth; it stays on disk after the run as the input record.
