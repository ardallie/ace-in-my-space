...
- /ace-plan-start <path-to-ace-agent-pre-planner-report.md> — generates a plan from a pre-plan analysis report; resolved decisions and `### Confirmed` findings become constraints, `[blocking]` open-questions feed the Phase 3 interview; `[deferrable]` ones become drafter-directed 'resolve in the plan' constraint lines
...
Runs outside Claude Code's plan mode. The brief arrives as free text, local files, or a GitHub issue (Phase 1's issue route). A `ace-plan-drafter` subagent reads the codebase and writes the plan to a deterministic path under `.ace/plans/`.

The `ace-plan-drafter` subagent receives Read, Glob, Grep, Bash, and Write — the set declared in its `tools:` frontmatter (Bash is constrained to read-only use by the agent prompt). The `acceptEdits` permission mode does not enforce a per-path restriction at the platform level; the subagent's prompt is the binding constraint — it writes only the single plan file.

Beyond the pipeline's own artefacts, this skill depends on the following sibling files; a port of the plan pipeline must carry this set: `${CLAUDE_PLUGIN_ROOT}/agents/ace-plan-drafter.md`, `${CLAUDE_PLUGIN_ROOT}/agents/ace-plan-validate.md` (the subagent Phase 7 spawns directly — not the standalone `/ace-plan-validate` skill at `${CLAUDE_PLUGIN_ROOT}/skills/ace-plan-validate/SKILL.md`, which this skill never invokes or reads), `${CLAUDE_PLUGIN_ROOT}/agents/ace-plan-revisor.md`, `${CLAUDE_PLUGIN_ROOT}/skills/ace-report-publish/SKILL.md`, `${CLAUDE_PLUGIN_ROOT}/skills/ace-run-interview/SKILL.md`, the shared files `${CLAUDE_PLUGIN_ROOT}/skills/plan-shared/identifiers.md`, `${CLAUDE_PLUGIN_ROOT}/skills/plan-shared/pre-planner-detection.md`, and `${CLAUDE_PLUGIN_ROOT}/skills/plan-shared/resolution-markers.md`, this skill's own `references/issue-route.md` and `references/pre-planner-transform.md`, and the rules files `${CLAUDE_PLUGIN_ROOT}/rules/environment.md`, `${CLAUDE_PLUGIN_ROOT}/rules/principles.md`, and `${CLAUDE_PLUGIN_ROOT}/rules/output-style.md`.
...
- **Issue input (plain brief)** — treated as a single-path input over the staged file, with one addition: the `Source: <staged path>` line is immediately followed by an `Issue: <issue URL>` line naming the origin. The drafter cites the issue, not the transport file (see `${CLAUDE_PLUGIN_ROOT}/agents/ace-plan-drafter.md`)
...
- Pre-planner report input — derive the slug from the report's `Title:` field:
  ...
  - **Sync note** — the title-derivation rule is single-sourced in `${CLAUDE_PLUGIN_ROOT}/skills/plan-shared/identifiers.md`, which `${CLAUDE_PLUGIN_ROOT}/agents/ace-plan-drafter.md` step 4 also consumes. The unit-marker append is parent-side only — for slug-derived inputs the drafter receives a fully-formed target path and never appends unit markers itself.
...
- `subagent_type`: `ace:ace-plan-drafter`
...
- `subagent_type`: `ace:ace-plan-validate`
...
1. `cp <draft path> <v1 path>` via Bash. On Windows MINGW64, both paths must be in POSIX form (`/c/...`) per `${CLAUDE_PLUGIN_ROOT}/rules/environment.md`.
...
- Otherwise, spawn the `ace-plan-revisor` subagent via `Agent`:

  - `subagent_type`: `ace:ace-plan-revisor`
  - `model`: `{Tier-3}`
...
- Do not invoke `EnterPlanMode` or `ExitPlanMode` — reachable here: unlike the three agent files (`ace-plan-drafter`, `ace-plan-revisor`, `ace-plan-validate`), this orchestrator carries neither a `tools:` allowlist nor `permissionMode` frontmatter, so nothing else forecloses plan mode at the platform level; those three drop this bullet because their own frontmatter already does.
