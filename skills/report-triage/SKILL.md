...
- **Stop and ask** only when relatively equal alternative paths exist, or a blocking decision needs user input that Phase 3's load-bearing questions did not already resolve. Test whether alternatives are relatively equal: alternatives whose costs sit an order of magnitude apart, or one of which breaks a documented contract, are not equal — resolve them from code and proceed. When genuinely equal, present the options per the option-list rules in `${CLAUDE_PLUGIN_ROOT}/skills/run-interview/SKILL.md` and end the turn. The apply pass then runs on the user's "fix" / "apply" or option choice.
