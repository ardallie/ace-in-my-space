...
- On Windows MINGW64 the `rev:path` argument form is rewritten by MSYS — apply the `MSYS_NO_PATHCONV=1` guidance in `${CLAUDE_PLUGIN_ROOT}/rules/environment.md` wherever this command uses `git show <sha>:<path>`.
...
Write the report to `.ace/reports/{yyyyMMdd}-{HHmm}-ace-agent-code-review-{suffix}.md` per the save procedure in `${CLAUDE_PLUGIN_ROOT}/skills/agent-shared/report-conventions.md` `## Save report` (`{suffix}` is the Phase 3 hex suffix).
