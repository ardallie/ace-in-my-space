...
the absence. Paths written `${CLAUDE_PLUGIN_ROOT}/...` are suite-internal and always
...
- Under `--checks skip` the parent runs no battery at all, this gate included --
  ...
  (`${CLAUDE_PLUGIN_ROOT}/skills/agent-shared/verification-checks.md`, `skip`). The package handoff
...
- A surface barred from inlining stays un-inlined -- the `ace-report-publish` title pipeline
...
- Subset packages that enumerate a file must all be updated together:
  `ace-agent-code-review`'s own files appear in `relay-workflow` **and**
...
- The rules-file registries are part of this class: the in-scope lists in
  ...
  `${CLAUDE_PLUGIN_ROOT}/skills/agent-shared/sibling-exchange-protocol.md`, and the
