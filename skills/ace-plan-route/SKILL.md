...
`/ace-plan-route [<artefact path> ... | <issue>]`

No flags. Resolve the scope from the arguments, first match wins, following the positional
rules in `${CLAUDE_PLUGIN_ROOT}/skills/agent-shared/input-conventions.md` -- read that file only when a
...
- **Issue number** -- a numeric token is a GitHub issue; fetch it per the **Issue number**
  branch of `## Positional parsing` in `${CLAUDE_PLUGIN_ROOT}/skills/agent-shared/input-conventions.md` and
...
When the user loads an issue with `/ace-plan-read <issue>` and directs a phase or subpackage
conversationally (e.g. `/ace-plan-read 957` followed by "handle phase 1"), treat that direction
...
2. Choose exactly one route. A **whole package** (an L1 wpak package file or its issue) routes
   ...
   wpak:entry` alias) and its printed `/ace-agent-scope` invocation as the entry point --
   `/ace-agent-scope`'s own off-ramp handles a pre-planner-sized package with one confirmation
...
7. Print the entry point as the final line of the response, then end the turn -- do not run the
   route. For scope, pre-planner, and plan-direct this is the ready-to-run `/ace-agent-scope`,
   `/ace-agent-pre-planner`, or `/ace-plan-start` command (for a wpak artefact, assembled via
...
- **scope** (`/ace-agent-scope`, whole packages only) -- the artefact is a complete wpak package:
...
- **pre-planner** (`/ace-agent-pre-planner`) -- multiple sequential phases, an unresolved or
...
- **plan-direct** (`/ace-plan-start`) -- one plannable surface, low/medium risk, no unresolved
...
- **Answerable from code** -- resolve through inspection and state the resolution in the
  ...
  `## Open questions` are deliberately carried unanswered, because `/ace-agent-scope`'s elicitation
  machinery is their resolver and the envelope asserts no source truth. Classify them, report
  the classification, and pass them to `/ace-agent-scope` unresolved -- a resolution stated here
...
- **Load-bearing** -- changes the route choice or the entry point and cannot be resolved from
  code alone. Run `/ace-run-interview` to surface it before presenting the recommendation.
...
enter the skill at all. On an explicit `/ace-plan-route` that also names a route, the user has
...
Reached from a call site -- invoked by `/ace-plan-read` step 5, or read by reference from the wpak
