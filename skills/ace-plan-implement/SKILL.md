...
2. Inspect the trailing `## Validation audit` block in the plan body, if present.
   ...
   - If `N > 0`, print the count and the report path, then ask via `AskUserQuestion`: "Proceed with `<N>` unresolved questions?" with options:
